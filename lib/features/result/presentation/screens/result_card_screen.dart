...
final resultCardExamsProvider = FutureProvider.autoDispose<List<Map<String, dynamic>>>((ref) async {
  ...
  return List<Map<String, dynamic>>.from(
    await c.from('exams').select('*').eq('school_id', sid).order('exam_date', ascending: false),
  );
});
...
final resultCardStudentsProvider = FutureProvider.autoDispose<List<Map<String, dynamic>>>((ref) async {
  ...
  if (sid == null) return [];
  return List<Map<String, dynamic>>.from(
    await c.from('students').select('id,full_name,admission_number,class_name,section_name').eq('school_id', sid).eq('is_active', true).order('full_name'),
  );
});
...
final resultCardDataProvider = FutureProvider.autoDispose.family<Map<String, dynamic>, String>((ref, key) async {
  ...
  if (st.isEmpty) throw Exception('Student not found.');
  ...
  final rows = mine.map((r) {
    final es = examSubjects.firstWhere(
      (x) => (x['id'] as num?)?.toInt() == (r['exam_subject_id'] as num?)?.toInt(),
      orElse: () => <String, dynamic>{},
    );
    final sj = subjectRows.firstWhere(
      (x) => (x['id'] as num?)?.toInt() == (es['subject_id'] as num?)?.toInt(),
      orElse: () => <String, dynamic>{},
    );
  ...
  return {
    ...
    'percentage': pct,
    'grade': grade(pct),
    'position': got > 0 ? scores.indexOf(got) + 1 : 0,
    'className': className,
    'section': section,
  };
});
...
class _ResultCardScreenState extends ConsumerState<ResultCardScreen> {
  ...
  Widget _body(BuildContext context, List<Map<String, dynamic>> exams, List<Map<String, dynamic>> students) {
    return ListView(
      ...
      children: [
        const Text('Result Card', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900)),
        const SizedBox(height: 5),
        const Text('Generate, preview and print the final student result card.'),
          ...
          children: [
            SizedBox(
              ...
              child: DropdownButtonFormField<int>(
                value: examId,
                decoration: const InputDecoration(labelText: 'Examination', border: OutlineInputBorder()),
                items: exams.map((e) => DropdownMenuItem<int>(value: (e['id'] as num).toInt(), child: Text((e['title'] ?? 'Examination').toString()))).toList(),
            ...
            SizedBox(
              ...
              child: DropdownButtonFormField<int>(
                ...
                items: students.map((s) => DropdownMenuItem<int>(value: (s['id'] as num).toInt(), child: Text((s['full_name'] ?? 'Student').toString()))).toList(),
        ...
        child: Column(
          ...
          children: [
            Center(child: Column(children: [
              Text('RESULT CARD', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w900, color: AppColors.primary)),
              Text('${data['exam']['title'] ?? 'Examination'}', style: const TextStyle(fontWeight: FontWeight.w700)),
            ])),
            const Divider(height: 30),
            Wrap(spacing: 28, runSpacing: 8, children: [
              Text('Student: ${st['full_name']}'),
              Text('Admission: ${st['admission_number'] ?? '—'}'),
              Text('Class: ${data['className']} ${data['section']}'),
              Text('Position: ${data['position'] > 0 ? data['position'] : '—'}'),
            ]),
            ...
            Table(
              ...
              children: [
                const TableRow(children: [
                  ...
                  Padding(padding: EdgeInsets.all(8), child: Text('Obtained')),
                  Padding(padding: EdgeInsets.all(8), child: Text('Total')),
                  Padding(padding: EdgeInsets.all(8), child: Text('Grade')),
                ]),
                ...rows.map((r) => TableRow(children: [
                  Padding(padding: const EdgeInsets.all(8), child: Text(r['subject'].toString())),
                  Padding(padding: const EdgeInsets.all(8), child: Text(r['obtained'].toString())),
                  Padding(padding: const EdgeInsets.all(8), child: Text(r['total'].toString())),
                  Padding(padding: const EdgeInsets.all(8), child: Text(r['grade'].toString())),
                ])),
              ],
            ),
            const SizedBox(height: 18),
            Text('Total: ${data['obtained']} / ${data['total']}'),
            Text('Percentage: ${(data['percentage'] as num).toStringAsFixed(2)}%'),
            Text('Overall Grade: ${data['grade']}'),
            const SizedBox(height: 18),
            Align(
      ...
      pw.Page(
        ...
        build: (_) => pw.Column(
          ...
          children: [
            ...
            pw.Text('Student: ${st['full_name']}'),
            pw.Text('Admission: ${st['admission_number'] ?? '—'}'),
            pw.Text('Class: ${data['className']} ${data['section']}'),
            pw.Text('Position: ${data['position'] > 0 ? data['position'] : '—'}'),
            pw.SizedBox(height: 15),
            pw.Table.fromTextArray(
              headers: ['Subject', 'Obtained', 'Total', 'Grade'],
              data: rows.map((r) => [r['subject'].toString(), r['obtained'].toString(), r['total'].toString(), r['grade'].toString()]).toList(),
            ),
            pw.SizedBox(height: 15),
            pw.Text('Total: ${data['obtained']} / ${data['total']}'),
            pw.Text('Percentage: ${(data['percentage'] as num).toStringAsFixed(2)}%'),
            pw.Text('Overall Grade: ${data['grade']}'),
          ],
        ),
      ),
    );
