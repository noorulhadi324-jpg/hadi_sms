...
class AuthGuard {
  ...
  static const Set<String> _teacherRoutes = {
    '/teacher-dashboard', '/attendance', '/assignment', '/examination', '/result', '/profile',
  };

  static const Set<String> _parentRoutes = {
    '/parent-dashboard', '/attendance', '/assignment', '/examination', '/result', '/finance', '/profile',
  };

  static const Set<String> _staffRoutes = {
    '/staff-dashboard', '/attendance', '/assignment', '/examination', '/result', '/finance', '/accounting', '/fee', '/library', '/profile',
  };
  ...
  static Future<String?> redirect(GoRouterState state) async {
    ...
    if (auth.isRecovering && location != '/reset-password') return '/reset-password';
    if (location == '/reset-password') return auth.session == null ? '/login' : null;

    if (location == '/splash' || _publicRoutes.contains(location)) {
      if (auth.session == null) return null;
      if (location == '/teacher-login' || location == '/parent-login' || location == '/staff-login') return null;
    ...
    if (location == '/teacher-dashboard' || location == '/parent-dashboard' || location == '/staff-dashboard') return '/dashboard';
    return null;
  }

  static Future<String?> _portalForCurrentUser() async => _portalForRole(await _currentRole());
  ...
  static Future<String?> _currentRole() async {
    ...
    try {
      final profile = await SupabaseConfig.client.from('profiles').select('role, is_active, school_id').eq('id', session.user.id).maybeSingle();
